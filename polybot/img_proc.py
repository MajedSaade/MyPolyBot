...
class S3Manager:
    ...
    def _initialize_s3_client(self) -> bool:
        ...
        try:
            ...
            logger.success(
                f"S3 client initialized successfully! Using region: {self.aws_region}, bucket: {self.bucket_name}")
            return True
            ...
            logger.error(
                "No AWS credentials found. Please attach IAM role to EC2 instance or configure AWS credentials.")
            return False
    ...
    def upload_file(self, local_path: Path, s3_key: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Upload the file
            logger.info(
                f"Uploading {local_path.name} ({local_path.stat().st_size} bytes) to S3 bucket {self.bucket_name}")
    ...
    def _log_missing_credentials(self):
        ...
        if not self._has_minimal_config():
            ...
            logger.warning(
                "AWS credentials not found. Please attach IAM role to EC2 instance or configure AWS credentials.")
